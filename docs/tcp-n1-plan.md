...
```c
// app/porter/prod/include/porter_type.h
...
static void *tcp_peer_recv_thread_func(void *arg)
{
    ...
    {
        /* tcp_recv_read_packet() で外側パケット ヘッダーを読み込む (既存ロジック流用) */
        ...
        /* +-- データ パケット: ctx->callback(service_id, peer->peer_id,
...
static void *tcp_peer_health_thread_func(void *arg)
{
    ...
    while (peer->tcp_health_running[path_idx] && peer->tcp_peer_running)
    {
        ...
        if (now - peer->tcp_last_ping_recv_ms[path_idx] > health_timeout_ms) {
            POTR_TRACE(INFO, "peer[%u] path[%d]: PING timeout, closing connection",
                     peer->peer_id, path_idx);
            /* ソケット クローズで recv スレッドを切断検知させる */
            POTR_SOCKET_CLOSE(peer->tcp_conn_fd[path_idx]);
            break;
        }
    }
...
static void receiver_accept_n1_loop(PotrContext *ctx, int path_idx)
{
    ...
    while (ctx->connect_thread_running[path_idx])
    {
        ...
        /* 接続元フィルター (既存 receiver_accept_loop と同一ロジック) */
        if (tcp_n1_is_src_filtered(ctx, path_idx, &peer_addr))
        {
            POTR_TRACE(POTR_TRACE_INFO, "connect_thread[service_id=%" PRId64 " path=%d]: "
        ...
        if (peer == NULL)
        {
            POTR_TRACE(POTR_TRACE_WARNING, "connect_thread[service_id=%" PRId64 " path=%d]: "
        ...
        POTR_TRACE(POTR_TRACE_VERBOSE, "connect_thread[service_id=%" PRId64 " path=%d]: "
                 "peer[%u] created",
...
case POTR_TYPE_TCP_BIDIR_N1:
{
    if (ctx->role == POTR_ROLE_RECEIVER)
    {
        ...
        /*    listen sock クローズで accept() をアン ブロック */
        for (i = 0; i < ctx->n_path; i++) {
