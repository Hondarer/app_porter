...
docs は明確に、ヘッダー 40 バイトは平文のまま AAD で保護すると説明している。  
参照: `app/porter/prod/docs/config.md:181-189`

したがって、`service_id`, `session_id`, `seq_num`, `ack_num`, `flags`, `payload_len`, `protocol_version` などのメタデータは観測可能である。改ざんは検知できても、**通信内容の全体像が秘匿されるわけではない**。
