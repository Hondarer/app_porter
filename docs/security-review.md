...
- docs は、DATA パケットのペイロードを `AES-256-GCM` で暗号化し、ヘッダ 40 バイトは平文のまま AAD で認証すると説明している。`PING / NACK / REJECT / FIN` は、ヘッダに対する 16 バイトの GCM タグのみを付与する。
...
docs は明確に、ヘッダ 40 バイトは平文のまま AAD で保護すると説明している。
