...
- **IPv6**: 非対応
- **バイト オーダー**: すべてのワイヤーフォーマットはビッグ エンディアン (ネットワーク バイト オーダー、NBO)
...
UDP と TCP で同一のヘッダーフォーマットを使用するため、将来の TCP↔UDP ブリッジ実装を容易にします。
...
ヘッダーフィールド (`elem_flags`・`payload_len`) はネットワーク バイト オーダー (NBO) で格納します。  
