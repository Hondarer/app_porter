...
static ConnectedThreadsCallState s_calls;
...
{
    (void)ctx;
    s_calls.tcp_send_ping_calls++;
    s_calls.last_tcp_send_ping_path = path_idx;
    return s_calls.tcp_send_ping_result;
}

static int fake_health_start(potr_context *ctx, int path_idx)
{
    (void)ctx;
    (void)path_idx;
    s_calls.health_start_calls++;
    return s_calls.health_start_result;
}
...
static void fake_join_recv(potr_context *ctx, int path_idx)
{
    (void)ctx;
    (void)path_idx;
    s_calls.join_recv_calls++;
}

static void fake_set_path_ping_state(potr_context *ctx, int path_idx, uint8_t next_state)
{
...
class potrConnectedThreadsTest : public Test
{
  protected:
    ...
    void SetUp() override
    {
        ...
        s_calls.recv_start_result = POTR_OK;
...
{
    // Arrange
    potr_internal_connected_threads_ops ops = make_ops(); // [状態] - fake ops 一式を用意する (send スレッドは未起動)。

    // Pre-Assert
    s_calls.recv_start_result = POTR_ERR_IO; // [Pre-Assert手順] - recv 開始 fake から POTR_ERR_IO を返却する。

    // Act
    int rtc = potr_internal_start_connected_threads(&ctx, 0, &ops); // [手順] - primary path (0) で接続時スレッド群を開始する。

    // Assert
    EXPECT_EQ(POTR_ERR_IO,
              rtc); // [確認_異常系] - potr_internal_start_connected_threads の戻り値が POTR_ERR_IO であること。
    EXPECT_EQ(1, s_calls.send_start_calls);    // [確認_異常系] - send 開始が 1 回呼ばれること。
    EXPECT_EQ(1, s_calls.recv_start_calls);    // [確認_異常系] - recv 開始が 1 回呼ばれること。
    EXPECT_EQ(1, s_calls.send_stop_calls);     // [確認_異常系] - この呼び出しで開始した send が停止されること。
...
TEST_F(potrConnectedThreadsTest, recv_failure_keeps_preexisting_send_thread_running)
{
    ...
    EXPECT_EQ(POTR_ERR_IO,
              rtc); // [確認_異常系] - potr_internal_start_connected_threads の戻り値が POTR_ERR_IO であること。
    EXPECT_EQ(0, s_calls.send_start_calls);    // [確認_異常系] - 既存 send があるため send 開始が呼ばれないこと。
    EXPECT_EQ(1, s_calls.recv_start_calls);    // [確認_異常系] - recv 開始が 1 回呼ばれること。
    EXPECT_EQ(0, s_calls.send_stop_calls);     // [確認_異常系] - 既存の send スレッドが停止されないこと。
    EXPECT_EQ(1, s_calls.close_conn_calls);    // [確認_異常系] - 接続が close されること。
    EXPECT_EQ(0, s_calls.tcp_send_ping_calls); // [確認_異常系] - bootstrap ping まで進まないこと。
}
...
TEST_F(potrConnectedThreadsTest, bootstrap_ping_failure_rolls_back_recv_and_new_send_thread)
{
    ...
    EXPECT_EQ(POTR_ERR_DISCONNECTED,
              rtc); // [確認_異常系] - potr_internal_start_connected_threads の戻り値が POTR_ERR_DISCONNECTED であること。
    EXPECT_EQ(1, s_calls.send_start_calls);                 // [確認_異常系] - send 開始が 1 回呼ばれること。
    EXPECT_EQ(1, s_calls.recv_start_calls);                 // [確認_異常系] - recv 開始が 1 回呼ばれること。
    EXPECT_EQ(1, s_calls.tcp_send_ping_calls);              // [確認_異常系] - bootstrap ping が 1 回呼ばれること。
    EXPECT_EQ(0, s_calls.health_start_calls);               // [確認_異常系] - health 開始まで進まないこと。
    EXPECT_EQ(1, s_calls.close_conn_calls);                 // [確認_異常系] - 接続が close されること。
...
TEST_F(potrConnectedThreadsTest, health_failure_rolls_back_recv_and_new_send_thread)
{
    ...
    EXPECT_EQ(POTR_ERR_OUT_OF_MEMORY,
              rtc); // [確認_異常系] - potr_internal_start_connected_threads の戻り値が POTR_ERR_OUT_OF_MEMORY であること。
    EXPECT_EQ(1, s_calls.send_start_calls);                 // [確認_異常系] - send 開始が 1 回呼ばれること。
    EXPECT_EQ(1, s_calls.recv_start_calls);                 // [確認_異常系] - recv 開始が 1 回呼ばれること。
    EXPECT_EQ(1, s_calls.tcp_send_ping_calls);              // [確認_異常系] - bootstrap ping が 1 回呼ばれること。
    EXPECT_EQ(1, s_calls.health_start_calls);               // [確認_異常系] - health 開始が 1 回呼ばれること。
    EXPECT_EQ(1, s_calls.close_conn_calls);                 // [確認_異常系] - 接続が close されること。
    EXPECT_EQ(1, s_calls.join_recv_calls);                  // [確認_異常系] - 起動済みの recv が join されること。
    EXPECT_EQ(1, s_calls.send_stop_calls);                  // [確認_異常系] - 新規に開始した send が停止されること。
    EXPECT_EQ(0, ctx.running[0]);                           // [確認_異常系] - path 0 の running フラグが下がること。
...
{
    // Arrange
    potr_internal_connected_threads_ops ops = make_ops(); // [状態] - fake ops 一式を用意する。
    ...
    EXPECT_EQ(POTR_ERR_IO,
              rtc); // [確認_異常系] - potr_internal_start_connected_threads の戻り値が POTR_ERR_IO であること。
    EXPECT_EQ(0, s_calls.send_start_calls);                 // [確認_異常系] - send 開始が呼ばれないこと。
    EXPECT_EQ(1, s_calls.recv_start_calls);                 // [確認_異常系] - recv 開始が 1 回呼ばれること。
...
{
    // Arrange
    potr_internal_connected_threads_ops ops = make_ops(); // [状態] - fake ops 一式を用意する (すべて成功を返す既定)。
    ...
    // Assert
    EXPECT_EQ(POTR_OK, rtc); // [確認_正常系] - potr_internal_start_connected_threads の戻り値が POTR_OK であること。
    EXPECT_EQ(1, s_calls.send_start_calls);        // [確認_正常系] - send 開始が 1 回呼ばれること。
    EXPECT_EQ(1, s_calls.recv_start_calls);        // [確認_正常系] - recv 開始が 1 回呼ばれること。
    EXPECT_EQ(1, s_calls.tcp_send_ping_calls);     // [確認_正常系] - bootstrap ping が 1 回呼ばれること。
    EXPECT_EQ(0, s_calls.last_tcp_send_ping_path); // [確認_正常系] - bootstrap ping の対象が path 0 であること。
    EXPECT_EQ(1, s_calls.health_start_calls);      // [確認_正常系] - health 開始が 1 回呼ばれること。
    EXPECT_EQ(0, s_calls.close_conn_calls);        // [確認_正常系] - close が呼ばれないこと。
    EXPECT_EQ(0, s_calls.join_recv_calls);         // [確認_正常系] - join が呼ばれないこと。
    EXPECT_EQ(0, s_calls.send_stop_calls);         // [確認_正常系] - send 停止が呼ばれないこと。
    EXPECT_EQ(1, s_calls.set_ping_state_calls);    // [確認_正常系] - ping 状態設定が 1 回呼ばれること。
    EXPECT_EQ(0, s_calls.last_set_ping_path);      // [確認_正常系] - 設定対象が path 0 であること。
