...
class potrSendQueueTest : public Test
{
  protected:
    ...
    {
        memset(&q, 0, sizeof(q));
        ASSERT_EQ(POTR_OK, potr_send_queue_init(&q, 4, 16));
    }
...
TEST_F(potrSendQueueTest, empty_queue_returns_empty_and_timeout)
{
    ...
    PotrPayloadElem elem; // [状態] - エントリを 1 件も積んでいない送信キューを用意する。

    // Pre-Assert

    // Act
    int rtc_peek = potr_send_queue_peek(&q, &elem);            // [手順] - 空キューを peek で参照する。
    int rtc_try_pop = potr_send_queue_try_pop(&q, &elem);      // [手順] - 空キューから try_pop で取り出す。
    int rtc_timed = potr_send_queue_peek_timed(&q, &elem, 10); // [手順] - timeout_ms=10 の peek_timed で待機する。

    // Assert
    EXPECT_EQ(POTR_ERR_EMPTY,
              rtc_peek); // [確認_異常系] - potr_send_queue_peek の戻り値が POTR_ERR_EMPTY であること。
    EXPECT_EQ(POTR_ERR_EMPTY,
              rtc_try_pop); // [確認_異常系] - potr_send_queue_try_pop の戻り値が POTR_ERR_EMPTY であること。
    EXPECT_EQ(POTR_ERR_TIMEOUT,
              rtc_timed); // [確認_異常系] - potr_send_queue_peek_timed の戻り値が POTR_ERR_TIMEOUT であること。
}
...
TEST_F(potrSendQueueTest, push_to_full_queue_returns_full)
{
    ...
    // Act
    int rtc_full =
        potr_send_queue_push(&q, 0, 0, payload, sizeof(payload)); // [手順] - 満杯のキューへ 5 件目を push する。

    // Assert
    EXPECT_EQ(POTR_ERR_FULL,
              rtc_full); // [確認_異常系] - 満杯時の potr_send_queue_push の戻り値が POTR_ERR_FULL であること。
}
...
TEST_F(potrSendQueueTest, push_wait_returns_canceled_when_stopped)
{
    ...
    // Act
    int rtc = potr_send_queue_push_wait(&q, 0, 0, payload, sizeof(payload),
                                        &running); // [手順] - 満杯のキューへ push_wait で追加を試みる。

    // Assert
    EXPECT_EQ(POTR_ERR_CANCELED,
              rtc); // [確認_異常系] - potr_send_queue_push_wait の戻り値が POTR_ERR_CANCELED であること。
}
...
TEST_F(potrSendQueueTest, pop_returns_canceled_when_stopped)
{
    ...
    // Act
    int rtc = potr_send_queue_pop(&q, &elem, &running); // [手順] - 空のキューから pop で取り出しを試みる。

    // Assert
    EXPECT_EQ(POTR_ERR_CANCELED,
              rtc); // [確認_異常系] - potr_send_queue_pop の戻り値が POTR_ERR_CANCELED であること。
}
