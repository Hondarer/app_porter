...
TEST_F(packetTest, packet_build_packed_sets_protocol_version)
{
    ...
    shdr.session_tv_nsec = 5678;

    ASSERT_EQ(POTR_SUCCESS, packet_build_packed(&pkt, &shdr, 9U, payload, sizeof(payload)));
    EXPECT_EQ(POTR_PROTOCOL_VERSION, ntohl(pkt.protocol_version));
}
...
TEST_F(packetTest, packet_parse_accepts_current_protocol_version)
{
    ...
    build_wire_packet(wire, POTR_PROTOCOL_VERSION);

    ASSERT_EQ(POTR_SUCCESS, packet_parse(&pkt, wire, sizeof(wire)));
    EXPECT_EQ(42, pkt.service_id);
    EXPECT_EQ(POTR_PROTOCOL_VERSION, pkt.protocol_version);
}
...
TEST_F(packetTest, packet_parse_rejects_different_protocol_version)
{
    ...
    build_wire_packet(wire, POTR_PROTOCOL_VERSION + 1U);

    EXPECT_EQ(POTR_ERROR, packet_parse(&pkt, wire, sizeof(wire)));
}
