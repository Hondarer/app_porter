...
com_util_mutex_init(&ctx.peers_mutex);
        ...
        resetTraceLevel();
        setTraceLevel("com_util_tracer_writef", TRACE_INFO);
    }
...
TEST_F(potrDisconnectPeerTest, handle_null)
{
    ...
    // Pre-Assert
    EXPECT_CALL(mock_log, com_util_tracer_writef(_, COM_UTIL_TRACE_LEVEL_ERROR, nullptr,
...
TEST_F(potrDisconnectPeerTest, normal_with_callback)
{
    ...
    EXPECT_CALL(mock_log, com_util_tracer_writef(_, COM_UTIL_TRACE_LEVEL_INFO, nullptr,
                                       AllOf(HasSubstr("potrDisconnectPeer.c"),
