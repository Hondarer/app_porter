...
TEST_F(windowTest, initInitializesStateAndReusesBuffersOnSameSize)
{
    ...
    // Act
    int rtc_null = potr_internal_window_init(NULL, 0U, 8U, 128U);    // [手順] - win に NULL を与えて呼び出す。
    int rtc_first = potr_internal_window_init(&win, 100U, 8U, 128U); // [手順] - initial_seq=100 で初期化する。
    first_packets = win.packets;
    int rtc_second = potr_internal_window_init(&win, 200U, 8U, 128U); // [手順] - 同一サイズ・initial_seq=200 で再初期化する。

    // Assert
    EXPECT_EQ(
        POTR_ERR_INVALID_ARGUMENT,
        rtc_null); // [確認_異常系] - win が NULL の場合に potr_internal_window_init の戻り値が POTR_ERR_INVALID_ARGUMENT であること。
    EXPECT_EQ(POTR_OK, rtc_first); // [確認_正常系] - potr_internal_window_init の戻り値から、初期化が成功したと判断できること。
    EXPECT_EQ(POTR_OK,
              rtc_second); // [確認_正常系] - potr_internal_window_init の戻り値から、同一サイズの再初期化が成功したと判断できること。
...
TEST_F(windowTest, sendPushEvictsOldestEntryWhenFull)
{
    ...
    int full_after_4 = potr_internal_window_send_full(&win); // [手順] - 4 件 push した直後の満杯判定を取得する。
    {
        ...
        int rtc_push = potr_internal_window_send_push(&win, &pkt); // [手順] - 満杯状態で 5 件目を push する。
        ASSERT_EQ(
            POTR_OK,
            rtc_push); // [確認_正常系] - potr_internal_window_send_push の戻り値から、5 件目の push が成功したと判断できること。
    }
    int rtc_evicted = potr_internal_window_send_get(&win, 0U, &out); // [手順] - evict された通番 0 を取得する。
    int rtc_latest = potr_internal_window_send_get(&win, 4U, &out);  // [手順] - 最新の通番 4 を取得する。

    // Assert
    EXPECT_EQ(1, full_after_4); // [確認_正常系] - window_size 件 push で満杯になること。
    EXPECT_EQ(
        POTR_ERR_NOT_FOUND,
        rtc_evicted); // [確認_異常系] - evict された通番 0 の potr_internal_window_send_get の戻り値が POTR_ERR_NOT_FOUND であること。
    EXPECT_EQ(
        POTR_OK,
        rtc_latest); // [確認_正常系] - potr_internal_window_send_get の戻り値から、最新エントリの取得が成功したと判断できること。
...
TEST_F(windowTest, sendGetReturnsDeepCopiedPayload)
{
    ...
    int rtc_get = potr_internal_window_send_get(&win, 0U, &out);
    int rtc_out_of_range = potr_internal_window_send_get(&win, 99U, &out); // [手順] - 範囲外の通番で取得する。

    // Assert
    EXPECT_EQ(POTR_OK,
              rtc_get); // [確認_正常系] - potr_internal_window_send_get の戻り値から、保持中の通番の取得が成功したと判断できること。
    EXPECT_EQ(0x01, out.payload[0]); // [確認_正常系] - push 時点のペイロードがディープ コピーで保持されること。
    EXPECT_EQ(
        POTR_ERR_NOT_FOUND,
        rtc_out_of_range); // [確認_異常系] - 範囲外の通番 99 の potr_internal_window_send_get の戻り値が POTR_ERR_NOT_FOUND であること。
}
...
TEST_F(windowTest, recvPushAndPopDeliversInOrder)
{
    ...
    // Act
    {
        ...
        int rtc_push0 = potr_internal_window_recv_push(&win, &pkt0); // [手順] - seq=0, 1 を順に push する。
        ASSERT_EQ(
            POTR_OK,
            rtc_push0); // [確認_正常系] - potr_internal_window_recv_push の戻り値から、seq=0 の push が成功したと判断できること。
        int rtc_push1 = potr_internal_window_recv_push(&win, &pkt1);
        ASSERT_EQ(
            POTR_OK,
            rtc_push1); // [確認_正常系] - potr_internal_window_recv_push の戻り値から、seq=1 の push が成功したと判断できること。
    }
    int rtc_pop0 = potr_internal_window_recv_pop(&win, &out);
    uint32_t seq0 = out.seq_num;
    int rtc_pop1 = potr_internal_window_recv_pop(&win, &out);
    uint32_t seq1 = out.seq_num;
    int rtc_pop_empty = potr_internal_window_recv_pop(&win, &out); // [手順] - 空になったウィンドウから pop する。

    // Assert
    EXPECT_EQ(POTR_OK,
              rtc_pop0); // [確認_正常系] - potr_internal_window_recv_pop の戻り値から、1 件目の pop が成功したと判断できること。
    EXPECT_EQ(0U, seq0); // [確認_正常系] - 1 件目が通番 0 で取り出されること。
    EXPECT_EQ(POTR_OK,
              rtc_pop1); // [確認_正常系] - potr_internal_window_recv_pop の戻り値から、2 件目の pop が成功したと判断できること。
    EXPECT_EQ(1U, seq1); // [確認_正常系] - 2 件目が通番 1 で取り出され、通番順が保たれること。
    EXPECT_EQ(POTR_ERR_EMPTY,
              rtc_pop_empty); // [確認_異常系] - 空ウィンドウの potr_internal_window_recv_pop の戻り値が POTR_ERR_EMPTY であること。
}
...
TEST_F(windowTest, recvOutOfOrderDetectsGapAndRecovers)
{
    ...
    // Act
    {
        ...
        int rtc_push2 = potr_internal_window_recv_push(&win, &pkt2); // [手順] - seq=2 を先行して push する。
        ASSERT_EQ(
            POTR_OK,
            rtc_push2); // [確認_正常系] - potr_internal_window_recv_push の戻り値から、seq=2 の push が成功したと判断できること。
    }
    int rtc_gap = potr_internal_window_recv_needs_nack(&win, &nack_num); // [手順] - 欠番判定を行う。
    int rtc_pop_blocked = potr_internal_window_recv_pop(&win, &out);     // [手順] - 欠番未解消のまま pop する。
    {
        potr_packet pkt0 = make_recv_packet(0U, payload, sizeof(payload));
        potr_packet pkt1 = make_recv_packet(1U, payload, sizeof(payload));
        int rtc_push0 = potr_internal_window_recv_push(&win, &pkt0); // [手順] - 欠番 seq=0, 1 を埋める。
        ASSERT_EQ(
            POTR_OK,
            rtc_push0); // [確認_正常系] - potr_internal_window_recv_push の戻り値から、seq=0 の push が成功したと判断できること。
        int rtc_push1 = potr_internal_window_recv_push(&win, &pkt1);
    ...
    int rtc_no_gap = potr_internal_window_recv_needs_nack(&win, &nack_num);
    int pop_count = 0;
    ...
    EXPECT_EQ(POTR_ERR_EMPTY,
              rtc_pop_blocked); // [確認_異常系] - 欠番未解消時の potr_internal_window_recv_pop の戻り値が POTR_ERR_EMPTY であること。
...
TEST_F(windowTest, recvPushRejectsOutOfWindowAndAcceptsDuplicate)
{
    ...
    int rtc_out = potr_internal_window_recv_push(&win, &pkt_out); // [手順] - base_seq + window_size の通番を push する。
    potr_packet pkt0 = make_recv_packet(0U, payload, sizeof(payload));
    int rtc_first = potr_internal_window_recv_push(&win, &pkt0);
    int rtc_dup = potr_internal_window_recv_push(&win, &pkt0); // [手順] - 同一通番を重複 push する。

    // Assert
    EXPECT_EQ(POTR_ERR_OUT_OF_WINDOW,
              rtc_out); // [確認_異常系] - 範囲外通番の potr_internal_window_recv_push の戻り値が POTR_ERR_OUT_OF_WINDOW であること。
    EXPECT_EQ(POTR_OK,
              rtc_first); // [確認_正常系] - potr_internal_window_recv_push の戻り値から、範囲内の push が成功したと判断できること。
    EXPECT_EQ(POTR_OK, rtc_dup); // [確認_正常系] - 重複 push が成功扱いになること。
}
...
TEST_F(windowTest, recvResetClearsSlotsAndSetsNewBase)
{
    ...
    int rtc_pop = potr_internal_window_recv_pop(&win, &out);
    ...
    EXPECT_EQ(
        POTR_ERR_EMPTY,
        rtc_pop); // [確認_正常系] - リセット後の potr_internal_window_recv_pop の戻り値が POTR_ERR_EMPTY となり、格納済みスロットが無効化されること。
}
