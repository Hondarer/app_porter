...
TEST_F(windowTest, initInitializesStateAndReusesBuffersOnSameSize)
{
    ...
    EXPECT_EQ(
        ...
        rtc_null); // [確認_異常系] - win が NULL の場合に window_init の戻り値が POTR_ERR_INVALID_ARGUMENT であること。
    EXPECT_EQ(POTR_OK, rtc_first); // [確認_正常系] - window_init の戻り値から、初期化が成功したと判断できること。
    EXPECT_EQ(POTR_OK,
...
TEST_F(windowTest, sendPushEvictsOldestEntryWhenFull)
{
    ...
    for (seq = 0U; seq < 4U; seq++)
    {
        ...
        ASSERT_EQ(POTR_OK, window_send_push(&win, &pkt));
    }
    ...
    EXPECT_EQ(
        POTR_ERR_NOT_FOUND,
        rtc_evicted); // [確認_異常系] - evict された通番 0 の window_send_get の戻り値が POTR_ERR_NOT_FOUND であること。
    EXPECT_EQ(
        POTR_OK,
...
TEST_F(windowTest, recvOutOfOrderDetectsGapAndRecovers)
{
    ...
    int pop_count = 0;
    while (window_recv_pop(&win, &out) == POTR_OK)
    {
        pop_count++;
    }
...
TEST_F(windowTest, recvPushRejectsOutOfWindowAndAcceptsDuplicate)
{
    ...
    EXPECT_EQ(POTR_ERR_OUT_OF_WINDOW,
              rtc_out); // [確認_異常系] - 範囲外通番の window_recv_push の戻り値が POTR_ERR_OUT_OF_WINDOW であること。
    EXPECT_EQ(POTR_OK,
