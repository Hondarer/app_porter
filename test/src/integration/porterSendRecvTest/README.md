...
受信側プロセス (`recv_h_`) に対して `interruptProcess()` で SIGINT を送信した後、`getStdout(recv_h_)` の内容を検証するテストで、CI 環境でのみ稀に失敗する事例が複数見つかりました。

送信側 (`send_h_`) がデータ送信後にプロンプト復帰・終了するまでを待ってから受信側へ `interruptProcess()` を発行するだけでは不十分です。暗号化 (復号) 処理や TCP ヘルスチェック確認など、受信側の処理に時間がかかる経路では、送信側の処理完了時点で受信側がまだ受信データを処理・標準出力へ反映し終えていないことがあります。ローカル環境では気づきにくく、CI のように負荷が高くスケジューリング遅延が起きやすい環境でのみ、受信側の出力反映より先に SIGINT が到着してしまい、`getStdout(recv_h_).find("<期待文字列>")` が `string::npos` になってテストが失敗します。

### 対処

`interruptProcess(recv_h_)` を呼び出す直前に、後段でアサーションする文字列を `ASSERT_NO_THROW(waitForOutput(recv_h_, "<期待文字列>", <timeout>));` で明示的に待機してください。送信側の終了待ち (`waitForExit(send_h_, ...)`) だけでは、受信側の出力タイミングを保証しません。
...
- `recv_h_`/`send_h_` いずれかの標準出力・標準エラーの内容を `getStdout()` / `getStderr()` でアサーションする場合、対象プロセスへの `interruptProcess()` や次の操作を行う前に、必ずその文字列を `waitForOutput()` で待機してから進めてください。
- `waitForExit()` は「プロセスが正常終了した」ことの保証にはなりますが、**別プロセスの出力反映** は保証しないことに注意してください。送信側の `waitForExit()` を待っても、受信側の出力が確定するとは限りません。
