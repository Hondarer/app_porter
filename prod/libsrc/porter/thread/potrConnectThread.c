...
static void set_tcp_path_ping_state(PotrContext *ctx, int path_idx, uint8_t next_state)
{
...
static void close_tcp_conn(PotrContext *ctx, int path_idx)
{
...
static void reconnect_wait(PotrContext *ctx, int path_idx, int wait_ms)
{
...
static void join_recv_thread(PotrContext *ctx, int path_idx)
{
...
static void reset_connection_state(PotrContext *ctx)
{
...
static void reset_all_paths_disconnected(PotrContext *ctx)
{
    ctx->peer_session_known = 0;
    ctx->frag_buf_len = 0;
...
static void reset_send_queue(PotrContext *ctx)
{
...
static int start_connected_threads(PotrContext *ctx, int path_idx)
{
    const PotrConnectedThreadsOps ops = {potr_send_thread_start,       potr_send_thread_stop, tcp_recv_thread_start,
                                         potr_tcp_health_thread_start, close_tcp_conn,        join_recv_thread,
...
static void stop_connected_threads(PotrContext *ctx, int path_idx)
{
...
static PotrSocket tcp_connect_with_timeout(PotrContext *ctx, int path_idx)
{
    PotrSocket sock;
...
static void sender_connect_loop(PotrContext *ctx, int path_idx)
{
...
static void receiver_accept_loop(PotrContext *ctx, int path_idx)
{
    ...
    while (ctx->connect_thread_running[path_idx])
    {
        ...
        /* 接続元フィルター: src_addr[path_idx] / src_port が指定されていれば一致確認 */
        {
            ...
            if (ctx->service.src_addr[path_idx][0] != '\0')
            {
                if (peer_addr.sin_addr.s_addr != ctx->src_addr_resolved[path_idx].s_addr)
                {
                    filtered = 1;
                }
            }
...
int potr_connect_thread_start(PotrContext *ctx)
{
    int i;
    ...
    if (ctx->role == POTR_ROLE_RECEIVER)
    {
        ...
        for (i = 0; i < ctx->n_path; i++)
        {
            ...
            ctx->tcp_first_pkt_buf[i] = (uint8_t *)malloc(PACKET_HEADER_SIZE + ctx->global.max_payload);
...
void potr_connect_thread_stop(PotrContext *ctx)
{
    int i;
    int any_running = 0;
