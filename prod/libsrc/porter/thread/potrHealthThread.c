...
static void signal_health_thread(PotrContext *ctx, int path_idx)
{
...
static void health_sleep(PotrContext *ctx, int path_idx, uint32_t interval_ms)
{
...
static int tcp_send_ping_packet(PotrContext *ctx, int path_idx)
{
    PotrPacket ping_pkt;
    ...
    while (ctx->health_running[0])
    {
        ...
        if (ctx->is_multi_peer)
        {
            ...
            for (i = 0; i < ctx->max_peers && ctx->health_running[0]; i++)
            {
                ...
                packet_build_ping(&ping_pkt, &peer_shdr, seq, health_states, (uint16_t)POTR_MAX_PATH);
...
int potr_health_thread_start(PotrContext *ctx)
{
    if (ctx == NULL)
    {
        return POTR_ERROR;
...
int potr_health_thread_stop(PotrContext *ctx)
{
    if (ctx == NULL)
    {
        return POTR_ERROR;
...
void potr_health_thread_wake(PotrContext *ctx)
{
...
void potr_tcp_health_thread_wake(PotrContext *ctx, int path_idx)
{
...
void potr_tcp_health_thread_wake_all(PotrContext *ctx)
{
    int i;
