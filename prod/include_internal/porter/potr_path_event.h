...
extern "C"
{
     ...
     */
    typedef struct potr_internal_prepared_path_events
    {
     ...
     */
    extern void potr_internal_callback_mutex_init(potr_context *ctx);

    /**
     *  @brief          コールバック直列化用ミューテックスを破棄します。
     *  @param[in,out]  ctx  対象のセッション コンテキストへのポインター。
     */
    extern void potr_internal_callback_mutex_dispose(potr_context *ctx);
     ...
     */
    extern void potr_internal_zero_path_states(int *states);
     ...
     */
    extern void potr_internal_copy_oneway_path_states(const potr_context *ctx, int *states);
     ...
     */
    extern void potr_internal_copy_bidir_udp_path_states(const potr_context *ctx, int *states);
     ...
     */
    extern void potr_internal_copy_bidir_n1_path_states(const potr_internal_peer_context *peer, int *states);
     ...
     */
    extern void potr_internal_copy_tcp_path_states(const potr_context *ctx, int *states);
     ...
     *  @param[out]     prepared    準備したイベント情報の格納先。
     *
     *  処理内容は potr_internal_sync_service_path_state_locked() のピア版です。peer の path_logical_alive と
     *  health_alive を更新します。\n
     ...
     *  @param[in,out]  ctx      対象のセッション コンテキストへのポインター。
     *  @param[in]      prepared potr_internal_sync_service_path_state_locked() が準備したイベント情報。
     *
     ...
     *  @param[in,out]  ctx      対象のセッション コンテキストへのポインター。
     *  @param[in]      peer     対象のピア コンテキストへのポインター。
     *  @param[in]      prepared potr_internal_sync_peer_path_state_locked() が準備したイベント情報。
     *
