 ...
 *  @version        1.0.0
 *
 *  PotrContext の実体定義。ライブラリ外部には公開しない。
 *
...
static inline PotrType potr_raw_base_type(PotrType t)
{
    ...
    case POTR_TYPE_UNICAST_RAW:
        return POTR_TYPE_UNICAST;
    case POTR_TYPE_MULTICAST_RAW:
        return POTR_TYPE_MULTICAST;
    case POTR_TYPE_BROADCAST_RAW:
        return POTR_TYPE_BROADCAST;
...
typedef struct PotrNackDedupEntry
{
 ...
 *  ピアテーブル (PotrContext::peers[]) に配置される。
 */
typedef struct PotrPeerContext
{
    ...
    com_util_local_lock *send_window_mutex; /**< send_window 保護 (送信・受信・ヘルスチェックスレッド競合)。 */
    ...
    volatile uint8_t path_ping_state
        [POTR_MAX_PATH]; /**< 自端の各パス PING 受信状態 (POTR_PING_STATE_*)。受信スレッドが更新、ヘルススレッドが読む。 */
    ...
    struct sockaddr_in dest_addr
        [POTR_MAX_PATH]; /**< 送信先ソケットアドレス (インデックス = ctx->sock[] の添字)。未使用スロットは sin_family == 0。 */
...
/**
 *  @brief  セッションコンテキスト構造体。PotrContext の実体。
 */
struct PotrContext
{
    PotrRecvCallback callback;                     /**< 受信コールバック。 */
    com_util_local_lock *callback_mutex;           /**< コールバック直列化用ミューテックス。 */
    com_util_thread *recv_thread[POTR_MAX_PATH];   /**< 受信スレッドハンドル (path ごと)。 */
    com_util_thread *health_thread[POTR_MAX_PATH]; /**< ヘルスチェックスレッドハンドル (path ごと、TCP: 全ロール)。 */
    com_util_local_lock *health_mutex[POTR_MAX_PATH]; /**< ヘルスチェックスレッド停止用ミューテックス (path ごと)。 */
    com_util_condvar *health_wakeup[POTR_MAX_PATH]; /**< ヘルスチェックスレッドを即時起床させる条件変数 (path ごと)。 */
    ...
    com_util_local_lock *
        send_window_mutex; /**< send_window 保護用ミューテックス (送信スレッド・ヘルスチェックスレッド・受信スレッドが競合するため)。 */
    ...
    volatile int
        health_running[POTR_MAX_PATH]; /**< ヘルスチェックスレッド実行フラグ (1: 実行中, 0: 停止)。path ごと。 */
    ...
    uint8_t
        *recv_buf; /**< 受信バッファ / 再送 wire 組立バッファ (動的確保。PACKET_HEADER_SIZE + max_payload バイト)。 */
    uint8_t *
        send_wire_buf; /**< 送信 wire 組立バッファ (動的確保。PACKET_HEADER_SIZE + max_payload バイト)。送信スレッドのみ使用。 */

    /* 非同期送信 (POTR_ROLE_SENDER のみ使用) */
    com_util_thread *send_thread;     /**< 送信スレッドハンドル。 */
    ...
    com_util_local_lock *peers_mutex; /**< ピアテーブル保護用ミューテックス。 */
    ...
    com_util_local_lock *tcp_send_mutex
        [POTR_MAX_PATH]; /**< TCP send() 排他制御 (path ごと)。送信スレッド・ヘルスチェックスレッド・recv スレッド競合防止。 */

    /* recv_window 保護 (TCP v2: 複数 recv スレッドが同一 recv_window にアクセスするため) */
    com_util_local_lock *recv_window_mutex; /**< recv_window 保護用ミューテックス。 */
    ...
    com_util_local_lock
        *tcp_state_mutex;           /**< tcp_state_cv 保護用ミューテックス。tcp_active_paths のカウンタ更新も保護。 */
    com_util_condvar *tcp_state_cv; /**< 切断通知・reconnect sleep の中断用条件変数。 */
    com_util_local_lock *tcp_close_mutex; /**< tcp_close_cv 保護用ミューテックス。 */
    com_util_condvar *tcp_close_cv;       /**< FIN_ACK 待機解除用条件変数。 */
    ...
    int buf_full_suppress_cnt
        [POTR_MAX_PATH]; /**< path ごとの送信バッファ満杯ログ抑制カウンタ (0: 抑制なし、1-10: 抑制中)。 */
    ...
    uint8_t *tcp_first_pkt_buf
        [POTR_MAX_PATH]; /**< 先読みパケットバッファ (動的確保、PACKET_HEADER_SIZE + max_payload バイト)。 */
