...
extern "C"
{
     ...
     */
    com_util_tracer *potr_internal_trace_get(void);
     ...
     */
    void potr_internal_trace_socket_failure_at(const char *file, int line, com_util_trace_level level,
 ...
 *  @par            例
    @code{.c}
    POTR_TRACE(COM_UTIL_TRACE_LEVEL_INFO,    "potr_service_open: service_id=%" PRId64 "", service_id);
    POTR_TRACE(COM_UTIL_TRACE_LEVEL_WARNING, "NACK received: seq=%u", seq);
    POTR_TRACE(COM_UTIL_TRACE_LEVEL_ERROR,   "socket() failed");
    @endcode
 */
#define POTR_TRACE(level, ...) com_util_tracer_writef(potr_internal_trace_get(), (level), NULL, __VA_ARGS__)
