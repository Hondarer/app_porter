...
ネットワーク サーバーが複数のクライアントを同時に処理する方法として、プロセス ベースのアプローチがある。各接続を独立したプロセスで処理することで、1 つのクライアントの処理が他に影響を与えない堅牢な構造を実現できます。
...
`fork()` を呼び出すと、親プロセスのメモリ空間がコピーされ、ファイル記述子も複製される。親と子で以下のように処理を分ける。
...
Windows には `fork()` がないため、`CreateProcess()` で自分自身を子プロセスとして再起動します。ソケット ハンドルは `--child <handle>` の形式でコマンド ライン引数に渡す。

ソケット ハンドルを子プロセスで使用するには、以下の 2 点が必要。
...
起動時にあらかじめワーカー プロセスを生成しておき、接続を処理するモデル。ワーカーは処理完了後も終了せず、次の接続を待つ。
...
Linux では複数の子プロセスが同じ listen ソケットに対して `accept()` を呼び出すことができます。カーネルが自動的に 1 つのプロセスだけを選んで接続を渡すため、親プロセスによる明示的な振り分けが不要。
...
4. 接続が来ると、カーネルが待機中のワーカー 1 つを起こす
...
Windows では `fork()` がないため、親プロセスが `accept()` を行い、空きワーカーにソケット ハンドルを渡す方式を採用している。

ワーカーとの通信には名前付きパイプを使用します。ワーカーは `--worker <pipe_name> --conns-per-worker <N>` 引数付きで起動される。
...
各ワーカーが `PeekNamedPipe()` でパイプを非ブロッキング チェックをしながら `select()` でアクティブなソケットを監視するイベント ループを使用します。接続が閉じるたびに完了通知を 1 バイト送信します。
...
両モデルとも、子プロセスは親の標準入出力を継承します。子プロセスの `printf()` は親と同じコンソールに出力される。

### Linux

`fork()` は親のファイル記述子をすべて複製します。stdin (fd 0), stdout (fd 1), stderr (fd 2) も自動的に継承される。

### Windows

`CreateProcess()` でコンソール アプリケーションを起動する場合、子プロセスはデフォルトで親のコンソールを共有します。`STARTUPINFO` の `dwFlags` に `STARTF_USESTDHANDLES` を設定していなければ、標準ハンドルも自動的に継承される。
...
| メモリ使用量 | 接続数に比例 | 一定 |
| 実装の複雑さ | シンプル | やや複雑 |
| 過負荷時の挙動 | 新規 fork が遅くなります。 | 新規接続がキューで待機 |
...
`--conns-per-worker N` (N ≥ 2) では、1 ワーカーが N 本の接続を同時に保持できます。ワーカー数 × N を超える接続が来ると、listen キューに留まる。
